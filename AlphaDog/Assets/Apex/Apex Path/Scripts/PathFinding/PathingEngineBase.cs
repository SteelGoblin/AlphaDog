...
namespace Apex.PathFinding
{
    ...
    public abstract class PathingEngineBase : IPathingEngine
    {
        ...
        private PathingStatus StartPathSegment()
        {
            ...
            if (options.usePathSmoothing && fromGrid == toGrid && !options.preventDiagonalMoves && PathSmoother.CanReducePath(fromPos, toPos, _segmentRequest.requesterProperties, fromGrid.cellMatrix, _cellCostStrategy))
            {
                var pathSegment = new Path(2);
                pathSegment.Push(toPos);
                pathSegment.Push(fromPos);
