...
public class SelectIndicator : MonoBehaviour {
    ...
    void Update()
    {
        if (mm.selectObject != null)
        {
            // check if it's a  hero vs. something else
            hc = mm.selectObject.GetComponent<HeroController>();
            if (hc)
            {
                hc.isHover = true;
                oldHC = hc;
                currSelect = true;
                Bounds bigBounds = mm.selectObject.GetComponentInChildren<Renderer>().bounds;

                // This "diameter" only works correctly for relatively circular or square objects
                float diameter = bigBounds.size.z;
                diameter *= 1.25f;
                Vector3 v = mm.selectObject.transform.position;
                v.y = 1f;
